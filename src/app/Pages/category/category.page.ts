...
import { CrearCategoriaComponent } from 'src/app/Components/crear-categoria/crear-categoria.component';
...
export class CategoryPage implements OnInit {
  ...
  async cargarCategorias() {
    const loading = await this.loadingCtrl.create({ message: 'Cargando categorías...' });
    await loading.present();

    const token = localStorage.getItem('authToken');

    this.http.get('http://127.0.0.1:8000/api/categories/categoriesList', {
  ...
  async crearCategoria() {
    const modal = await this.modalCtrl.create({ component: CrearCategoriaComponent });
  ...
  async confirmDelete(categoria: any) {
    ...
    this.http.delete(`http://127.0.0.1:8000/api/categories/categorieRemove/${categoria.id}`, {
      headers: { Authorization: `Bearer ${token}` }
    }).subscribe({
      next: async (res: any) => {
        await loading.dismiss();
