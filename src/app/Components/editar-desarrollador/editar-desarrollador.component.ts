...
@Component({
  ...
  <ion-toolbar style="background:#f9f9f9; color:#222; font-family:'Times New Roman', serif;">
    <ion-title style="font-weight:700; font-size:1.5rem;">Editar Desarrollador</ion-title>
  ...
  <ion-card style="border-radius:12px; box-shadow:0 4px 12px rgba(0,0,0,.08);">
    <ion-card-content>
      ...
      <ion-item fill="outline" style="border-radius:8px; margin-bottom:15px;">
        <ion-label position="floating" style="font-weight:600; color:#0c1a2b;">Nombre del Desarrollador</ion-label>
  ...
  async editar() {
    if (!this.categoria.nombre.trim()) {
      const toast = await this.toastCtrl.create({
        message: 'El nombre del desarrollador es obligatorio',
        duration: 2000,
        color: 'warning'
      });
      await toast.present();
    ...
    const loading = await this.loadingCtrl.create({ message: 'Actualizando desarrollador...' });
    await loading.present();

    const token = localStorage.getItem('authToken');
    const body = { name: this.categoria.nombre };
    const url = `https://143.110.205.116/api/developers/developerUpdate/${this.categoria.id}`;
    ...
    this.http.put(url, body, {
      ...
      error: async (err) => {
        ...
        const toast = await this.toastCtrl.create({
          message: 'Error al actualizar desarrollador',
          duration: 2000,
          color: 'danger'
        });
        await toast.present();
