...
export class EditarPlataformaComponent  {
 @Input() categoria: any;
  ...
  async editar() {
    ...
    const loading = await this.loadingCtrl.create({ message: 'Actualizando plataforma...' });
    await loading.present();

    const token = localStorage.getItem('authToken');
    const body = { name: this.categoria.nombre };
    const url = `http://127.0.0.1:8000/api/platforms/platformUpdate/${this.categoria.id}`;
    ...
    this.http.put(url, body, {
      ...
      error: async (err) => {
        ...
        const toast = await this.toastCtrl.create({
          message: 'Error al actualizar plataforma',
          duration: 2000,
          color: 'danger'
        });
        await toast.present();
